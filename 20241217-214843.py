...
class Admin(User):
    ...
    def search_equipment(self):
        subprocess.run('clear')
        print("\n--- Search Equipment ---")
        search_name = input("Enter equipment name to search: ").strip().lower()

        matched_equipment = [
            equipment for equipment in system.equipment_list
            if search_name in equipment.name.lower()
        ]

        if not matched_equipment:
            print("No equipment found with that name.")
            input("Press Enter to continue...")
            return
        ...
        self.manage_found_equipment(matched_equipment)

    def manage_found_equipment(self, equipment_list):
        while True:
            try:
                choice = int(input("\nEnter the number of equipment to manage (or 0 to go back): ")) - 1
                if choice == -1:
                    return

                if 0 <= choice < len(equipment_list):
                    selected_equipment = equipment_list[choice]
    ...
    def update_or_delete_equipment(self, equipment):
        while True:
            ...
            if choice == "1":
                ...
                self.delete_selected_equipment(equipment)
                return
            elif choice == "0":
                return
            else:
    ...
    def delete_selected_equipment(self, equipment):
    ...
    def update_equipment(self, category):
        ...
        try:
            ...
            if 0 <= choice < len(filtered_equipment):
                ...
                print(f"Updating {equipment_to_update.name}:")
...
class System:
    ...
    def run(self):
        while True:
            subprocess.run('clear')
            print("\n--- Welcome to Film Equipment Rental---")
            print("1. Login")
